...
local function init_xterm_colors()
   ...
   -- we usually bump a few very dark xterm colors to brighter values to improve
...
-- default_foreground_color and background_color can be mud color codes or MUSHclient's raw numeric color values
   ...
   if section:find(CODE_PREFIX, nil, true) then
      ...
      for i = first_i, num_tokens - 1, 2 do
         ...
         if code == XTERM_CODE then                         -- xterm 256 colors
            ...
            -- Mud treats x1...x15 as normal ANSI colors.
...
-- Convert Mud and short x codes to 3 digit x codes
...
-- Tries to convert ANSI sequences to Mud color codes
...
function ColoursToANSI(text)
   ...
   if text:find(CODE_PREFIX, nil, true) then
      ...
      text = text:gsub(X_DIGITS_CAPTURE_PATTERN, function(a)
         ...
         -- Mud treats x1...x15 as normal ANSI codes
